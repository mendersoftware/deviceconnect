    ...
    def test_connect(self, clean_mongo, tenant_id=None):
        ...
        with dev.connect() as dev_conn:
            ...
            with management_api_connect(dev.id, tenant_id=tenant_id) as user_conn:
                user_conn.send(msgpack.dumps({"cmd": "/bin/sh", "data": None}))
                msg = dev_conn.recv()
                assert msgpack.loads(msg) == {"cmd": "/bin/sh", "data": None}
                dev_conn.send(msgpack.dumps({"cmd": "/bin/sh", "data": b"sh-5.0$ "}))
                msg = user_conn.recv()
                assert msgpack.loads(msg) == {"cmd": "/bin/sh", "data": b"sh-5.0$ "}
